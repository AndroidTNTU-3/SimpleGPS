...
public class PolyUtil {
    ...
    public static LatLng GetTargetPoint(LatLng point, List<LatLng> polyline,
            boolean geodesic, double tolerance){
    	isLocationOnPath(point, polyline, geodesic, tolerance);
    	return targetPoint;
    }
    ...
    public static Boolean isLocationOnEdgeOrPath(LatLng point, List<LatLng> poly, boolean closed,
        ...
        int size = poly.size();
        Log.i("DEBUG", String.valueOf(poly.size()));
        if (size == 0) {
            return false;
        }
                ...
                if (max(lat1, lat2) >= minAcceptable && min(lat1, lat2) <= maxAcceptable) {
                    ...
                    for (double x3 : xTry) {
                        ...
                        if (havDist < havTolerance) {
                        	Log.i("DEBUG", "In UTIL:" + Double.toString((point2.latitude)));
